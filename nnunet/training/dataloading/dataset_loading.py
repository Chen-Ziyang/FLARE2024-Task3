...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
                 ...
                 pad_sides=None, ori_shape=(96, 448, 448)):
        """
        ...
        self.ori_data_shape = (self.batch_size, 1, *ori_shape)
    ...
    def determine_shapes(self):
        ...
        num_color_channels = max(1, case_all_data.shape[0] - 1)
    ...
    def generate_train_batch(self):
        ...
        data_ori = np.zeros(self.ori_data_shape, dtype=np.float32)
        ...
        for j, i in enumerate(selected_keys):
            ...
            zoom_factors = (self.ori_shape[0] / case_all_data[0].shape[0],
                            self.ori_shape[1] / case_all_data[0].shape[1],
                            self.ori_shape[2] / case_all_data[0].shape[2])
            data_ori[j] = np.expand_dims(zoom(case_all_data[0], zoom_factors, order=1), axis=0)
            ...
            if case_all_data.shape[0] == 1:
                data[j] = np.pad(case_all_data[:], ((0, 0),
                                                    (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                    (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                    (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
                                 self.pad_mode, **self.pad_kwargs_data)

                seg[j, 0] = np.pad(case_all_data[:], ((0, 0),
                                                      (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                      (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                      (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
        ...
        return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys, 'data_ori': case_all_data}
