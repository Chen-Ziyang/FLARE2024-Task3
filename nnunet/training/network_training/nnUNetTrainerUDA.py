...
from nnunet.training.network_training.network_trainer_uda import NetworkTrainerUDA
...
class nnUNetTrainerUDA(NetworkTrainerUDA):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        """
        super(nnUNetTrainerUDA, self).__init__(deterministic, fp16)
        ...
        print("using plans_file ", plans_file)
        ...
        self.folder_with_preprocessed_data_target = None

        # set in self.initialize()

        self.dl_tar = self.dl_tr = self.dl_val = None
        ...
        self.online_eval_foreground_dc_ct = []
        self.online_eval_tp_ct = []
        self.online_eval_fp_ct = []
        self.online_eval_fn_ct = []
        ...
        self.MR_task = 'Task999_PseudoMR'   # 'Task999_PseudoMR'/'Task777_MR'
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        if training:
            ...
            self.tr_gen, self.val_gen = get_default_augmentation(self.dl_tr, self.dl_val,
                                                                 ...
                                                                 self.data_aug_params)
            self.target_gen = get_default_augmentation(self.dl_tar, None,
                                                       self.data_aug_params[
                                                       'patch_size_for_spatialtransform'],
                                                       self.data_aug_params)
    ...
    def plot_network_architecture(self):
        ...
        self.print_to_log_file(self.generator)
        self.print_to_log_file(self.discriminator)
        self.print_to_log_file("\n")
        # try:
    ...
    def run_training(self):
        ...
        super(nnUNetTrainerUDA, self).run_training()
    ...
    def load_dataset(self):
        ...
        self.dataset_target = load_dataset(self.folder_with_preprocessed_data_target)
    ...
    def get_basic_generators(self):
        ...
        if self.threeD:
            dl_tr = DataLoader3D(self.dataset_tr, self.basic_generator_patch_size, self.patch_size, self.batch_size,
                                 ...
                                 pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r', ori_shape=(96, 448, 448))
            dl_val = DataLoader3D(self.dataset_val, self.patch_size, self.patch_size, self.batch_size, False,
                                  oversample_foreground_percent=self.oversample_foreground_percent,
                                  pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
            dl_tar = DataLoader3D(self.dataset_tar, self.basic_generator_patch_size, self.patch_size, self.batch_size,
                                  False, oversample_foreground_percent=self.oversample_foreground_percent,
                                  pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r', ori_shape=(96, 448, 448))
        else:
            ...
            dl_val = DataLoader2D(self.dataset_val, self.patch_size, self.patch_size, self.batch_size,
                                  oversample_foreground_percent=self.oversample_foreground_percent,
                                  pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
            dl_tar = DataLoader2D(self.dataset_tar, self.basic_generator_patch_size, self.patch_size, self.batch_size,
                                  oversample_foreground_percent=self.oversample_foreground_percent,
                                  pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
        return dl_tr, dl_val, dl_tar
    ...
    def finish_online_evaluation(self):
        ...
        self.online_eval_tp_ct = np.sum(self.online_eval_tp_ct, 0)
        self.online_eval_fp_ct = np.sum(self.online_eval_fp_ct, 0)
        self.online_eval_fn_ct = np.sum(self.online_eval_fn_ct, 0)

        global_dc_per_class_ct = [i for i in [2 * i / (2 * i + j + k) for i, j, k in
                                  zip(self.online_eval_tp_ct, self.online_eval_fp_ct, self.online_eval_fn_ct)]
                                  if not np.isnan(i)]
        self.all_val_eval_metrics.append(np.mean(global_dc_per_class_ct))
        self.print_to_log_file("Average global CT foreground Dice:", [np.round(i, 4) for i in global_dc_per_class_ct])
        ...
        if self.MR_task == 'Task999_PseudoMR':
            self.print_to_log_file("Average global Pseudo-MR foreground Dice:", [np.round(i, 4) for i in global_dc_per_class])
        else:
            self.print_to_log_file("Average global CT-2-MR foreground Dice:", [np.round(i, 4) for i in global_dc_per_class])
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        super(nnUNetTrainerUDA, self).save_checkpoint(fname, save_optimizer)
        info = OrderedDict()
