...
class NetworkTrainerUDA(object):
    def __init__(self, deterministic=True, fp16=False):
        ...
        self.target_gen = self.tr_gen = self.val_gen = None
        ...
        self.dataset_target = self.dataset = None  # these can be None for inference mode
        self.dataset_tar = self.dataset_tr = self.dataset_val = None  # do not need to be used, they just appear if you are using the suggested load_dataset_and_do_split
    ...
    def do_split(self):
        ...
        tar_keys = list(self.dataset_target.keys())
        ...
        self.dataset_tar = OrderedDict()
        for i in tar_keys:
            self.dataset_tar[i] = self.dataset_target[i]
    ...
    def plot_progress(self):
        ...
        try:
            ...
            if len(self.all_tr_losses) == len(x_values):
                ax.plot(x_values, self.all_tr_losses, color='b', ls='-', label="loss_tr")
            if len(self.all_val_losses) == len(x_values):
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        ...
        if self.discriminator is not None:
            ...
            for key in dis_state_dict.keys():
                dis_state_dict[key] = dis_state_dict[key].cpu()

        lr_sched_state_dct = None
        if self.lr_scheduler is not None and hasattr(self.lr_scheduler, 'state_dict'):
            lr_sched_state_dct = self.lr_scheduler.state_dict()

        gen_lr_sched_state_dct = None
        if self.gen_lr_scheduler is not None and hasattr(self.gen_lr_scheduler, 'state_dict'):
            gen_lr_sched_state_dct = self.gen_lr_scheduler.state_dict()

        dis_lr_sched_state_dct = None
        if self.dis_lr_scheduler is not None and hasattr(self.dis_lr_scheduler, 'state_dict'):
            dis_lr_sched_state_dct = self.dis_lr_scheduler.state_dict()

        if save_optimizer:
            optimizer_state_dict = self.optimizer.state_dict()
            gen_optimizer_state_dict = None
            if self.gen_optimizer is not None:
                gen_optimizer_state_dict = self.gen_optimizer.state_dict()
            dis_optimizer_state_dict = None
        ...
        save_this = {
            ...
            'dis_optimizer_state_dict': dis_optimizer_state_dict,
            'lr_scheduler_state_dict': lr_sched_state_dct,
            'gen_lr_scheduler_state_dict': gen_lr_sched_state_dct,
            'dis_lr_scheduler_state_dict': dis_lr_sched_state_dct,
            ...
            if self.use_progress_bar:
                with trange(self.num_batches_per_epoch) as tbar:
                    for b in tbar:
                        ...
                        l = self.run_iteration(self.tr_gen, self.target_gen, True)
    ...
    def run_iteration(self, data_generator, tar_data_generator, do_backprop=True, run_online_evaluation=False):
