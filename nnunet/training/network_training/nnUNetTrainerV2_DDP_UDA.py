...
from nnunet.training.network_training.nnUNetTrainerV2_FLARE import nnUNetTrainerV2_FLARE_Big
...
class nnUNetTrainerV2_DDP_UDA(nnUNetTrainerV2_FLARE_Big):
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        if not self.was_initialized:
            ...
            self.folder_with_preprocessed_data = join(self.dataset_directory, self.plans['data_identifier'] +
                                                      "_stage%d" % self.stage)
            self.folder_with_preprocessed_data_target = '/data1/userdisk0/zychen/flare2024/' + \
                                                        self.MR_task + '/nnUNetData_plans_FLARE24Big_stage0'
            # self.folder_with_preprocessed_data_target = '/data1/userdisk0/zychen/nnUNet_raw_data_base/Task999_PseudoMR/' \
            #                                             'nnUNetData_plans_FLARE24Big_stage0'

            if training:
                self.dl_tr, self.dl_val, self.dl_tar = self.get_basic_generators()
                if self.unpack_data:
                    if self.local_rank == 0:
                        print("unpacking dataset")
                        unpack_dataset(self.folder_with_preprocessed_data)
                        unpack_dataset(self.folder_with_preprocessed_data_target)
                        print("done")
                    distributed.barrier()
                else:
                    print(
                ...
                if self.MR_task == 'Task999_PseudoMR':
                    self.target_gen = get_moreDA_augmentation_target_wlabel(
                        self.dl_tar,
                ...
                self.print_to_log_file("VALIDATION KEYS:\n %s" % (str(self.dataset_val.keys())),
                                       also_print_to_console=False)
                self.print_to_log_file("TRAGET DOMAIN KEYS:\n %s" % (str(self.dataset_tar.keys())),
                                       also_print_to_console=False)
            else:
                pass
            ...
            checkpoint = torch.load('/data1/userdisk0/zychen/RESULTS_FOLDER/nnUNet/3d_fullres/Task666_FLARE2024Challenge/'
                                    'nnUNetTrainerV2_FLARE_Big_DDP__nnUNetPlansFLARE24Big_alpha<1_1000epoch_0.71/all/'
                                    'model_final_checkpoint.model', map_location=torch.device('cpu'))
            if self.MR_task == 'Task999_PseudoMR':
            ...
            if self.generator is not None:
                if self.MR_task == 'Task999_PseudoMR':
                    new_state_dict = OrderedDict()
                    curr_state_dict_keys = list(self.generator.state_dict().keys())
                    for k, value in checkpoint['gen_state_dict'].items():
                        key = k
                        if key not in curr_state_dict_keys and key.startswith('module.'):
                            key = key[7:]
                        new_state_dict[key] = value
    ...
    def run_iteration(self, data_generator, target_generator, do_backprop=True, run_online_evaluation=False):
    ...
    def run_another_online_evaluation(self, output, target):
        with torch.no_grad():
            num_classes = output[0].shape[1]
            output_seg = output[0].argmax(1)
            target = target[0][:, 0]
        ...
        fp_hard = fp_hard.detach().cpu().numpy().sum(0)
        fn_hard = fn_hard.detach().cpu().numpy().sum(0)
        self.online_eval_foreground_dc_ct.append(list((2 * tp_hard) / (2 * tp_hard + fp_hard + fn_hard + 1e-8)))
        self.online_eval_tp_ct.append(list(tp_hard))
        self.online_eval_fp_ct.append(list(fp_hard))
        self.online_eval_fn_ct.append(list(fn_hard))
    ...
    def run_training(self):
        ...
        while self.epoch < self.max_num_epochs:
            ...
            train_losses_epoch = []
            seg_losses_epoch = []
            consis_s_losses_epoch = []
            consis_c_losses_epoch = []
            gen_losses_epoch = []
            dis_losses_epoch = []
            ...
            if self.use_progress_bar:
                with trange(self.num_batches_per_epoch) as tbar:
                    for b in tbar:
                        ...
                        if self.MR_task == 'Task999_PseudoMR':
                            l = self.run_iteration(self.tr_gen, self.target_gen, True, True)
                            tbar.set_postfix(loss=l)
                            train_losses_epoch.append(l)
                        else:
                            l, seg_l, gen_l, dis_l, consis_s_l, consis_c_l, ct_to_mr, ct, alpha, ct_properties, mr_properties\
                                = self.run_iteration(self.tr_gen, self.target_gen, True, True)

                            tbar.set_postfix(loss=l)
                            train_losses_epoch.append(l)
                            seg_losses_epoch.append(seg_l)
                            consis_s_losses_epoch.append(consis_s_l)
                            consis_c_losses_epoch.append(consis_c_l)
                            gen_losses_epoch.append(gen_l)
                            dis_losses_epoch.append(dis_l)

                            if b != tbar[-1]: del ct, ct_to_mr, ct_properties, mr_properties
                ...
                for b in range(self.num_batches_per_epoch):
                    if self.MR_task == 'Task999_PseudoMR':
                        ...
                        if np.isnan(seg_l): self.print_to_log_file("NaN seg loss : %.4f" % seg_l)
                        if np.isnan(consis_s_l): self.print_to_log_file("NaN consis style loss : %.4f" % consis_s_l)
                        if np.isnan(consis_c_l): self.print_to_log_file("NaN consis content loss : %.4f" % consis_c_l)
                        if np.isnan(gen_l): self.print_to_log_file("NaN gen loss : %.4f" % gen_l)
                        if np.isnan(dis_l): self.print_to_log_file("NaN dis loss : %.4f" % dis_l)

                        if b != (self.num_batches_per_epoch-1): del ct, ct_to_mr, ct_properties, mr_properties

            self.all_tr_losses.append(np.mean(train_losses_epoch))
            self.print_to_log_file("train loss : %.4f" % self.all_tr_losses[-1])

            if self.MR_task != 'Task999_PseudoMR':
                self.print_to_log_file("seg loss : %.4f" % np.mean(seg_losses_epoch))
                self.print_to_log_file("consis style loss : %.4f" % np.mean(consis_s_losses_epoch))
                self.print_to_log_file("consis content loss : %.4f" % np.mean(consis_c_losses_epoch))
                self.print_to_log_file("gen loss : %.4f" % np.mean(gen_losses_epoch))
                self.print_to_log_file("dis loss : %.4f" % np.mean(dis_losses_epoch))
                self.print_to_log_file("alpha : %.4f" % alpha)

                if (self.epoch+1) % 5 == 0 or self.epoch == 0:
                    nii_file = sitk.GetImageFromArray(ct_to_mr[0][0])
                    nii_file.SetSpacing(ct_properties[0]['itk_spacing'])
                    nii_file.SetOrigin(ct_properties[0]['itk_origin'])
                    nii_file.SetDirection(ct_properties[0]['itk_direction'])
                    sitk.WriteImage(nii_file, join(self.output_folder, str(self.epoch) + "-ct2mr.nii.gz"))
                    sleep(3)
                    self.print_to_log_file('[ct2mr: ', sitk.GetArrayFromImage(nii_file).max(),
                                                       sitk.GetArrayFromImage(nii_file).min(), ']')
                    del nii_file

                    nii_file = sitk.GetImageFromArray(ct[0][0])
                    nii_file.SetSpacing(ct_properties[0]['itk_spacing'])
                    nii_file.SetOrigin(ct_properties[0]['itk_origin'])
                    nii_file.SetDirection(ct_properties[0]['itk_direction'])
                    sitk.WriteImage(nii_file, join(self.output_folder, str(self.epoch) + "-ct.nii.gz"))
                    sleep(3)
                    self.print_to_log_file('[ct: ', sitk.GetArrayFromImage(nii_file).max(),
                                                    sitk.GetArrayFromImage(nii_file).min(), ']')
                    del nii_file
                del ct_to_mr, ct, ct_properties, mr_properties
            # with torch.no_grad():
    ...
    def load_checkpoint_ram(self, checkpoint, train=True):
        ...
        # gen net
        if self.generator is not None:
            gen_new_state_dict = OrderedDict()
            gen_curr_state_dict_keys = list(self.generator.state_dict().keys())
            # if state dict comes from nn.DataParallel but we use non-parallel model here then the state dict keys do not
            # match. Use heuristic to make it match
            for k, value in checkpoint['gen_state_dict'].items():
                key = k
                if key not in gen_curr_state_dict_keys:
                    print("duh")
                    key = key[7:]
                gen_new_state_dict[key] = value

        # dis net
        if self.discriminator is not None:
            dis_new_state_dict = OrderedDict()
            dis_curr_state_dict_keys = list(self.discriminator.state_dict().keys())
            # if state dict comes from nn.DataParallel but we use non-parallel model here then the state dict keys do not
            # match. Use heuristic to make it match
            for k, value in checkpoint['dis_state_dict'].items():
                key = k
                if key not in dis_curr_state_dict_keys:
                    print("duh")
                    key = key[7:]
                dis_new_state_dict[key] = value
        ...
        self.network.load_state_dict(new_state_dict)
        if self.generator is not None:
            self.generator.load_state_dict(gen_new_state_dict)
        if self.discriminator is not None:
            self.discriminator.load_state_dict(dis_new_state_dict)
        ...
        if train:
            ...
            if self.gen_optimizer is not None:
                optimizer_state_dict = checkpoint['gen_optimizer_state_dict']
                if optimizer_state_dict is not None:
                    self.gen_optimizer.load_state_dict(optimizer_state_dict)

            if self.dis_optimizer is not None:
                optimizer_state_dict = checkpoint['dis_optimizer_state_dict']
                if optimizer_state_dict is not None:
                    self.dis_optimizer.load_state_dict(optimizer_state_dict)
            ...
            if self.gen_lr_scheduler is not None:
                if self.gen_lr_scheduler is not None and hasattr(self.gen_lr_scheduler, 'load_state_dict') and checkpoint[
                    'gen_lr_scheduler_state_dict'] is not None:
                    self.lr_scheduler.load_state_dict(checkpoint['gen_lr_scheduler_state_dict'])
                if issubclass(self.gen_lr_scheduler.__class__, _LRScheduler):
                    self.gen_lr_scheduler.step(self.epoch)

            if self.dis_lr_scheduler is not None:
                if self.dis_lr_scheduler is not None and hasattr(self.dis_lr_scheduler, 'load_state_dict') and checkpoint[
                    'dis_lr_scheduler_state_dict'] is not None:
                    self.dis_lr_scheduler.load_state_dict(checkpoint['dis_lr_scheduler_state_dict'])
                if issubclass(self.dis_lr_scheduler.__class__, _LRScheduler):
                    self.dis_lr_scheduler.step(self.epoch)
